...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  if(req.body.role === 'admin'){
    return next(new AppError('Admin signup is not allowed.', 403));
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if(!user){
    return next(new AppError('There is no user with that emails address.', 404));
  }
  ...
  try{
    ...
    res.status(200).json({
      status: 'success',
      message: 'Token sent to the emails!'
    })
  } catch (err){
    user.passwordResetToken = undefined;
    user.passwordResetExpires = undefined;
    await user.save( {validateBeforeSave: false} );
    return next(new AppError('There was an error sending the emails. Try again later!', 500));
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 4) Update changePasswordAt property for the user
